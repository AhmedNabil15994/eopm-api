...
class UPaymentService
{
    ...
    public function send($order, $payment, $userToken = '', $type = 'frontend',$total=null)
    {
        ...
        $extraMerchantsData['amounts'][0] =  $total ?? $order['total'];
        ...
        $fields = [
            ...
            'error_url' => $url['failed'].'?transaction='.encryptOrderId($order['id']).'&total='.($total ?? $order['total']),
            ...
            'reference' => $order['id'],
            'notifyURL' => route('api.orders.failed.upayment'),
            'total_price' => $total ?? $order['total'],
            'userToken' => $userToken,
        ];
    ...
    public function paymentUrls($type)
    {
        switch($type){
            ...
            case 'api':
                ...
                $url['failed'] = route('api.orders.failed.upayment');
                break;

            case 'wallet':
                $url['success'] = route('api.wallets.success.upayment');
                $url['failed'] = route('api.wallets.failed.upayment');
                break;
        }
        return $url;
    }
